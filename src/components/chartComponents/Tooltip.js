...
import { formatPriceUSD } from "../../utils/commonUtils";
...
const Tooltip = ({
    ...
    React.useEffect(() => {
        d3.select(anchorEl)
            .on("mouseout.chart_tooltip", () => {
                d3.select(ref.current).attr("opacity", 0);
            })
            .on("mouseover.chart_tooltip", () => {

                d3.select(ref.current).attr("opacity", 1);
            })
            .on("mousemove.chart_tooltip", () => {
                d3.select(ref.current)
                    .selectAll(".tooltipLinePoint")
                    .attr("opacity", 1);
                followPoints();
    ...
    return (
        <g ref={ref} opacity={0} {...props}>
            ...
            <g className="tooltipContent">
                <rect className="contentBackground" rx={4} ry={4} opacity={0.3} />
